...
def run_pipeline(mode: str):
    ...
    dest_url = get_dest_url()  # 🔥 CORRIGIDO AQUI
    ...
    try:
        db_utils.recreate_database_if_not_exists(
            get_server_url(),
            get_dest_db_name()
        )
    ...
    for schema in schemas:
        ...
        ordered_tables = db_utils.build_dependency_graph(
            src_engine, tables, schema
        )
                    ...
                    if mode == "FULL_ANON":
                        for row in rows:
                            for col, method in treatments.items():
                                if method == "ANONYMIZE" and row.get(col) is not None:
                                    row[col] = anonymizer.anonymize_value(
                                        col, row[col]
                                    )
...
if __name__ == "__main__":
    ...
    print("=" * 50)
    print("1. Migração Pura")
    print("2. Migração + Anonimização")
    print("=" * 50)
    ...
    if mode_env:
        mode = "MIGRATE_ONLY" if mode_env == "1" else "FULL_ANON"
        run_pipeline(mode)
        sys.exit(0)
    ...
    try:
        ...
        print("Modo não interativo detectado. Usando FULL_ANON")
        escolha = "2"

    if escolha == "1":
        run_pipeline("MIGRATE_ONLY")
    elif escolha == "2":
        run_pipeline("FULL_ANON")
    else:
